...
const createUser = async function (req, res) {
    ...
    if(!phoneRegex.test(data.phone)) return res
    .status(400)
    .send({
      status: false,
    ...
    if(!emailRegex.test(data.email)) return res
    .status(400)
    .send({
      status: false,
    ...
    if(data.password.trim().length>15||data.password.trim().length<8) return res.status(400).send({status:false,message:"Password length should be between 8 to 15 characters"})
    if(!passwordRegex.test(data.password)) return res
    .status(400)
    .send({
      status: false,
      message: "Password should contain uppercase, lowercase, special characters and a number",
    });

    const addressContains=["street", "city", "pincode"]
    for(field of addressContains){
      if(!isValidString(data.address[field])) return res
      .status(400)
      .send({
        status: false,
        message: `${field} is empty and must contain characters.`,
      });
    }
    ...
    for(field of uniqueFields){
      ...
      if(userData) return res.status(400).send({status:false,message:`${field} already taken!`})
    }
